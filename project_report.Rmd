 ...
 * `job_type` = type of employment during pregnancy (government, private company, non-profit, self-employed)
...
wfls_df = 
  ...
  mutate(
    ...
    family_income = case_when(
      ...
      pph1 == '1' ~ "TRUE", 
      pph1 == '2' ~ "FLASE"), 
   postpartum_check =  as.logical(postpartum_check), 
...
2) Length of maternity leave by co-parenting status 
```{r}
parent = 
  merged_wfls %>% 
  drop_na(partner) %>% 
...
5) 
PostPartum checkup and family income
```{r}
...
merged_wfls %>% 
  drop_na(family_income) %>% 
  drop_na(postpartum_check) %>% 
  mutate(postpartum_check = as.logical(postpartum_check)) 


  group_by(family_income, postpartum_check) %>% 
  summarize(
    total = sum(postpartum_check == "yes", "no"),  
    yes = (sum(postpartum_check == "yes")), 
    prop = (yes/total))
